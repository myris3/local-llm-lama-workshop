...
const downloadModel = async (modelName) => {
  pipe = await pipeline("text2text-generation", modelName, {device: 'webgpu'});
  self.postMessage({
    status: "ready",
  });
};
...
const generateResponse = async (content) => {
  ...
  const prompt = messagesToPrompt(content);
  const output = await pipe(prompt, { max_new_tokens: 200, temperature: 1.0, repetition_penalty: 2.0, no_repeat_ngram_size: 3 });
